...
def _process_extension_chunk(
    ...
    voice_block_parts.append(
        ...
        "persona précise :\n\n"
        "1. **Densité ET pattern d'emojis vs la signature** : compare la fréquence "
        "d'emojis que tu vas produire avec ce que dit la `Signature de voix` ci-dessus. "
        "Si elle décrit une persona qui en utilise peu, n'en ajoute pas par habitude. "
        "Si elle en utilise beaucoup, ne les retire pas non plus.\n"
        "**Mais surtout : VARIÉTÉ STRUCTURELLE du pattern, jamais mécanique**. Le piège "
        "le plus reconnaissable d'un LLM est d'appliquer le même pattern emoji sur tous "
        "les tweets — toujours 3 à la fin, toujours 1 au même endroit, toujours en "
        "cluster. **C'est interdit.** Concrètement, sur n'importe quel batch de tweets "
        "de cette persona :\n"
        "- Le nombre d'emojis varie tweet à tweet (0, 1, 2, 3, 4… selon la matière du "
        "tweet précis).\n"
        "- La position varie : fin de tweet (le plus courant), milieu d'une phrase "
        "pour ponctuer un mot, début de tweet comme un cri visuel, entre deux phrases.\n"
        "- Des tweets ENTIERS sans aucun emoji sont obligatoires régulièrement — "
        "surtout pour les pics de lucidité froide ou les thèses tranchantes — c'est ce "
        "contraste qui fait que les emojis ailleurs ont du poids.\n"
        "- Pas deux tweets consécutifs avec exactement le même pattern emoji (même "
        "nombre + même position).\n"
        "**Test mental** : si on peut prédire le nombre et la position des emojis d'un "
        "tweet en regardant juste les 5 précédents, tu es en pattern robotique. Casse-le.\n\n"
